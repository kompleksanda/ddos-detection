...
class PacketSink(object):
    ...
    def put(self, pkt):
        if not self.selector or self.selector(pkt):
            ...
            if self.debug:
                print(pkt)
            self.get_entropy(pkt)
            #self.get_entropy_time(pkt, now)

    
    def get_entropy(self, packet):
        ...
        if len(self.ip_list) == self.entropy_window:
            ...
            #print("entropy=",entropy, len_all_ip, abs(len_all_ip - self.entropy_window))
    ...
    def get_entropy_time(self, packet, now):
        self.ip_list.append(packet.src)
        if packet.src not in self.all_ip: self.all_ip.append(packet.src)
        if len(self.ip_list) < self.entropy_window:
            return
...
class SwitchPort(object):
    ...
    def put(self, pkt):
        ...
        if self.limit_bytes and tmp_byte_count >= self.qlimit:
            ...
            self.byte_size = tmp_byte_count
            return self.store.put(pkt)
        if self.packets_drop > self.qlimit:
            print("---------------------------")
